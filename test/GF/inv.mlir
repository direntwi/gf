...
module {
  ...
  func.func @gf_inv(%x: i8) -> i8
      attributes {
        "passthrough" = ["noinline", "optnone"],
        // optional, but nice: give it its own text section label
        llvm.section = ".text.gf_inv"
      } {
    %y = gf.inv %x : i8
    return %y : i8
  }
  ...
  func.func @main() -> i8 {
    %tmp = memref.alloca() : memref<i8>
    %c   = arith.constant 0x53 : i8
    memref.store %c, %tmp[] : memref<i8>
    %v   = memref.load %tmp[] : memref<i8>
    %r   = func.call @gf_inv(%v) : (i8) -> i8
    return %r : i8
  }
}
