...
module {
  // Put the mul body in its own section and forbid inlining.
  func.func @gf_mul(%x: i8, %y: i8) -> i8
      attributes {
                   llvm.noinline,
                   llvm.section = ".text.gf_mul" } {
    %z = gf.mul %x, %y : i8
    return %z : i8
  }

  // main: make args dynamic (store→load) so upstream can't constant-fold.
  func.func @main() -> i8
      {
    %ax = memref.alloca() : memref<i8>
    %bx = memref.alloca() : memref<i8>

    %c5 = arith.constant 5 : i8
    %c7 = arith.constant 7 : i8
    memref.store %c5, %ax[] : memref<i8>   // rank-0 memref needs []  ← important
    memref.store %c7, %bx[] : memref<i8>

    %a = memref.load %ax[] : memref<i8>
    %b = memref.load %bx[] : memref<i8>
    %r = func.call @gf_mul(%a, %b) : (i8, i8) -> i8
    return %r : i8
  }
}
