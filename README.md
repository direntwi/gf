...
This is an example of an out-of-tree [MLIR](https://mlir.llvm.org/) dialect along with a gf `opt`-like tool to operate on that dialect.
...
cmake -G Ninja `$LLVM_SRC_DIR/llvm` \
    ...
    -DLLVM_EXTERNAL_PROJECTS=gf-dialect -DLLVM_EXTERNAL_GF_DIALECT_SOURCE_DIR=../
cmake --build . --target check-gf
```
